...
export default function HubHistoryPage({ currentHub, hubOrders, updateOrderStatus, refreshOrders, navigate }) {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return list;
  }, [ordersForHub, orderSearch, statusFilter, paymentFilter, sortBy, jobByOrderId]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-3xl font-black tracking-tight text-slate-950">Hub Operations & History</h2>
      ...
      <Card className="relative left-1/2 w-[calc(100vw-1rem)] -translate-x-1/2 px-3 sm:w-[calc(100vw-2rem)] sm:px-4 lg:w-[min(1500px,calc(100vw-3rem))] lg:px-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h3 className="text-xl font-bold">Incoming & Historical Orders</h3>
            <p className="mt-2 text-sm text-slate-600">
              Search and filter all incoming, active, and completed hub orders.
            </p>
          </div>
          
          <div className="flex items-center gap-2 self-end">
            <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-600 bg-slate-100 px-3 py-2 rounded-xl cursor-pointer">
              <input
                type="checkbox"
          ...
          <table className="w-full min-w-[840px] table-fixed text-left text-sm border-collapse">
            <thead className="sticky top-0 bg-white z-10 shadow-sm border-b">
              <tr className="border-b text-xs uppercase tracking-wide text-slate-500 bg-slate-50">
                ...
                <th className="w-32 px-2 py-3">Agent</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {filteredOrders.map((item) => {
                ...
                return (
                  <tr key={item.id} className="align-top odd:bg-white even:bg-slate-50 hover:bg-slate-100/50 transition">
                    ...
                    <td className="px-2 py-4">
                      <select value={item.status} onChange={(e) => updateOrderStatus(item.id, e.target.value)} className="w-full rounded-xl border border-slate-200 px-2 py-1.5 text-xs font-semibold bg-white outline-none focus:ring-2 focus:ring-slate-300">
                      ...
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {canPauseOrder(item) && (
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-lg border bg-white px-2 py-1 text-xs font-semibold text-slate-700 disabled:opacity-50"
                          >
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        {paymentPending && (
                          <button
                            ...
                            className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-emerald-200 bg-white px-2 py-1.5 font-bold text-xs text-emerald-700 hover:bg-emerald-50 transition disabled:opacity-50"
                          >
                        ...
                        {sendEnabled && routeableAgents.length > 0 && (
                          <button
                            ...
                            className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-emerald-700 px-2.5 py-2 font-bold text-xs text-white hover:bg-emerald-600 disabled:bg-slate-300 transition"
                          >
                        ...
                        {sendEnabled && routeableAgents.length === 0 && (
                          <button
                            onClick={() => navigate("hubPrinters")}
                            className="rounded-xl border border-amber-200 px-2.5 py-1.5 text-left text-xs font-bold text-amber-800 bg-amber-50 hover:bg-amber-100 transition"
                          >
                            Open agent to queue.
                          </button>
                        )}
              ...
              {filteredOrders.length === 0 && (
                <tr>
                  <td colSpan={8} className="py-12 text-center text-sm text-slate-500 font-medium">
              ...
              {documentPreview && (
                <div className="rounded-2xl border bg-slate-50 p-3">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    ...
                    <button type="button" onClick={() => setDocumentPreview(null)} className="rounded-xl border bg-white px-3 py-2 text-xs font-semibold hover:bg-slate-50 transition">
                      Close preview
                    </button>
                  </div>
